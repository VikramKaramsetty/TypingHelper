...
import txt from './../SecondStep/Words.txt';
...
function Step2Game() {
  ...
  // const words = ["John", "Jim", "Bob", "Tom", "The", "Rollercoaster", "Roadway", "Hello", "King", "Broughjt"]
  /* =========== */

  let [pointer, setPointer] = useState(0);
  let [styles, setStyles] = useState([]);
  let [beforeStyle, setBeforeStyle] = useState([]);
  ...
  function updateWord() {
    ...
    setTimeout(() => { // adding a timeout so you can see last letter
      ...
      if (wordTimeout > 1000) {
        setWordTimeout(wordTimeout - 250);
      }
  ...
  const [wordTimeout, setWordTimeout] = useState(5000);

  useInterval(() => { // custom interval: https://overreacted.io/making-setinterval-declarative-with-react-hooks/
    // console.log("time");
    updateWord()
  }, wordTimeout)
  ...
  const fullWord = charString.map((letter, ind) => {
    ...
    if (pointer === words[wordInd].length) {
      ...
      for (let i = 0; i < beforeStyle.length; i++) {
        if (beforeStyle[i] !== "correct") {
          console.log(beforeStyle[i]) // styles is always unstyled because it is set to unstyled before
          isCorrect = false;
        }
      }

      if (wordTimeout > 1500 && isCorrect === true) {
        updateWord();
      }

      // updateWord();
      // DO NOT UPDATE WORD, you wait for the timeout to finish
